...
async def _public_reserve(
    client: AsyncClient, slug: str, number: int, *, phone: str, name: str
) -> Any:
...
async def test_dashboard_reflects_business_state(api_client: AsyncClient) -> None:
    ...
    _, slug_a = await _raffle(api_client, title="Rifa A", total=5, price=20000, publish=True)
    ...
    assert (
        await _public_reserve(api_client, slug_a, 1, phone="3001", name="Ana")
    ).status_code == 201
    assert (
        await _public_reserve(api_client, slug_a, 2, phone="3002", name="Beto")
    ).status_code == 201
