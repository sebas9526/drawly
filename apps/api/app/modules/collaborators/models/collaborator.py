...
class Collaborator(UUIDAuditBase, table=True):
    """A sales collaborator that belongs to a single raffle.

    Each raffle can have many collaborators; when a ticket is reserved/paid it may
    record which collaborator made the sale (see ``tickets.collaborator_id``).
